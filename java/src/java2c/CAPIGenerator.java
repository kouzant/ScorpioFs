...
public class CAPIGenerator
{
   ...
   public void generateClassAPI(PrintWriter hOut, PrintWriter cOut, boolean extraArgForInterfaces, boolean includeStaticFields)
   {
      ...
      for (int i = 0; i < constructors.length; i++)
      {
         ...
         if (Modifier.isPublic(constructor.getModifiers()))
         {
            ...
            constructor2name.put(constructor, getMethodName(null, "new", constructor.getParameterTypes()));
         }
      }
      ...
      for (int i = 0; i < methods.length; i++)
      {
         ...
         if (Modifier.isPublic(mod))
         {
            ...
            method2name.put(method, getMethodName(method.getReturnType(), method.getName(), method.getParameterTypes()));
         }
      }
   ...
   private String getMethodName(Class returnType, String methodName, Class[] argumentTypes)
   {
	  String prepend = (returnType != null ? getJVMTypeSignature(returnType) + "__" : "");
      if (argumentTypes == null || argumentTypes.length == 0)
         return mangle(prepend + methodName);
      else
         return mangle(prepend + methodName + "__" + getJVMTypeSignatures(argumentTypes));
   }
}
